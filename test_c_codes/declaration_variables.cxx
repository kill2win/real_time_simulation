...
double rd = i*f/3 +d;
cout << "i*f/3 +d, implicit " << c << rd << endl;
// explicit conversion to integer first
int ri = int(i)*int(f)/int(3) + int(d);
cout << "i*f/3 +d, integer " << c << ri << endl;
// what is the result from this calculation?
cout << "i/3*f +d, question? " << c << i/3*f +d << endl;
// here is a new one, an "enumerated" type, you can define what the
// names are
enum SimState {Running,Stopped};
